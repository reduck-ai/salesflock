...
// findQuotes — the one anchoring primitive: verbatim text → every raw span rendering to it. What
// `search_quotes` hands the judge; the judge never counts offsets, it cites text and gets spans back.
test("findQuotes: a unique quote → one span that slices back to it", () => {
	const hits = findQuotes(ev, "Founding PM at AgentAchieve");
	assert.equal(hits.length, 1);
	assert.equal(quoteText(ev, hits[0]), "Founding PM at AgentAchieve");
});

test("findQuotes: a repeated quote → every occurrence, in order", () => {
	const hits = findQuotes(ev, "San Francisco Bay Area"); // appears twice
	assert.equal(hits.length, 2);
	assert.ok(hits[0].start < hits[1].start);
	for (const q of hits) assert.equal(quoteText(ev, q), "San Francisco Bay Area");
});

test("findQuotes: absent text → []", () => {
	assert.deepEqual(findQuotes(ev, "Chief Robot Officer"), []);
});

test("findQuotes: a rendered quote maps to the raw span carrying the stripped `**`/markers", () => {
	// the judge cites the RENDERED text (no `**`, block markers collapsed); the span re-includes them.
	const hits = findQuotes(
		experiences,
		"title: Director of Product Management, AI company: Automation Anywhere"
	);
	assert.equal(hits.length, 1);
	assert.match(quoteText(experiences, hits[0]), /\*\*company:\*\*/);
});
